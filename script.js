document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      if (field.minLength && field.minLength > 0 && value.length < field.minLength) {
        showFieldError(field, getErrorMessage(field, 'minlength', field.minLength));
        isValid = false;
        if (!firstInvalidField) firstInvalidField = field;
        return;
      }
      ...
      if (field.maxLength && field.maxLength > 0 && value.length > field.maxLength) {
        showFieldError(field, getErrorMessage(field, 'maxlength', field.maxLength));
        isValid = false;
        if (!firstInvalidField) firstInvalidField = field;
        return;
      }
  ...
  function getErrorMessage(field, type, param) {
    ...
    return field.dataset['error' + capitalise(type)] || messages[type] || 'Invalid value.';
  }
  ...
  function getFieldLabel(field) {
    ...
    if (wrappingLabel) {
      return wrappingLabel.childNodes[0] &&
        wrappingLabel.childNodes[0].nodeType === Node.TEXT_NODE
        ? wrappingLabel.childNodes[0].textContent.trim().replace(/:$/, '')
        : null;
    }
    return null;
  }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "האטלייה של שרגא ווייל משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '12cb2ab3-13ec-4245-8ff4-d3953f190f76',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '12cb2ab3-13ec-4245-8ff4-d3953f190f76';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '12cb2ab3-13ec-4245-8ff4-d3953f190f76';
